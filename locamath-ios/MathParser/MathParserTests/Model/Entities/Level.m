...
{    
    if ([self.isAllTasksSolved boolValue]) {
        ...
        if (![self.isTest boolValue]) {
            ...
            description = [NSString stringWithFormat:@"%02i.%02i %@ (%@)", day, month, self.name, NSLocalizedString(@"Solved", @"Solved level/path")];
        } else {
            description = [NSString stringWithFormat:@"%02i.%02i %@ (%@)", day, month, self.name, NSLocalizedString(@"Got star", @"Got star for test level completion")];
        }
