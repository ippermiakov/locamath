...
- (void)onTapLevel:(id)sender
{
    ...
    if ([[TransitionsManager sharedInstance] canOpenTask:task error:&error]) {
        ...
        SolvingAndExercisesViewController *slovViewCintroller = [[SolvingAndExercisesViewController alloc] initWithAchievement:task];
        slovViewCintroller.task = task;
        slovViewCintroller.backDelegate = self;
        [self presentViewController:slovViewCintroller animated:YES completion:nil];
    } else {
