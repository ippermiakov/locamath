...
//                                                          }];
        ...
        //clear unlinked parent. it could occur due to unhandled app termination during registration               
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:AFNetworkingReachabilityDidChangeNotification
                                                      ...
                                                      usingBlock:^(NSNotification *note) {
                                                          if ([[MTHTTPClient sharedMTHTTPClient] isReachable]) {
                                                              ...
                                                              [MTHTTPClient sharedMTHTTPClient].isChildActivated = NO;
                                                              self.currentChild.isDataLoaded = @NO;
                                   ...
                                   failure:(ChildManagerFailureBlock)failureBlock
{
    ...
    NSLog(@"isReachable: %@ isChildActivated: %@", [[MTHTTPClient sharedMTHTTPClient] isReachable] ? @"YES":@"NO",
          [[MTHTTPClient sharedMTHTTPClient] isChildActivated] ? @"YES":@"NO");
        ...
        if ([[MTHTTPClient sharedMTHTTPClient] isReachable] && self.currentChild && ![DataUtils isCurrentChildDefault]) {
            [self setActiveChildWithID:[self.currentChild.identifier stringValue]
                               success:^(NSDictionary *dictionary) {
                                     ...
                                     weakSelf.currentChild.selectionDate = [NSDate date];
...
//                                     NSLog(@"updated currentChild: %@", weakSelf.currentChild);
            ...
            [[MTFileParser sharedInstance] parseLocalFilesToCoreDataForChild:self.currentChild completion:^{
                [MBProgressHUD hideHUDForWindow];
                                                        ...
                                                        success:^(NSDictionary *responseDictionary) {
                                                            ...
                                                            if ([newChild.isCurrent boolValue]) {
                                                                ...
                                                                [self setActiveChildWithID:[newChild.identifier stringValue]
                                                                                     success:^(NSDictionary *dictionary) {
                                                                                         newChild.selectionDate = [NSDate date];
...
{
    NSArray *sortedStatsTime = [[self.currentChild.spendTimes allObjects] sortedArrayUsingComparator:^NSComparisonResult(SpendTimeStatistic *obj1, SpendTimeStatistic *obj2) {
        return [obj1.statisticDate compare:obj2.statisticDate];
    }];
    
    if (sortedStatsTime.count) {
        SpendTimeStatistic *last = sortedStatsTime[0];
        
        if (![last.statisticDate isEqualToString:[DataUtils presentDateString]]) {
            NSMutableArray * param = [NSMutableArray new];
            [sortedStatsTime each:^(SpendTimeStatistic *sender) {
                [param addObject:[sender toDictionary]];
            }];
            
            [[MTHTTPClient sharedMTHTTPClient] updateSpendTimes:param withSuccess:^(BOOL finished, NSError *error) {
                
            [[[ChildManager sharedInstance].currentChild.spendTimes allObjects] makeObjectsPerformSelector:@selector(deleteEntity)];
            [[NSManagedObjectContext contextForCurrentThread] saveToPersistentStoreAndWait];
                
            NSLog(@"timers in app %@ ", [[ChildManager sharedInstance].currentChild.spendTimes allObjects]);
