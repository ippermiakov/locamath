...
@property (strong, nonatomic) NSMutableArray *helpAnimatioPositions;
@property (strong, nonatomic) UIImageView *animationImageArow;
@property (strong, nonatomic) UILabel *animationHelpLabel;

@property (strong, nonatomic) UIImageView *animationImageArowForError;
@property (strong, nonatomic) UILabel *animationHelpLabelForError;
...
+ (AnimationManager *)sharedInstance
{
    ...
    dispatch_once(&pred, ^{
        ...
        [sharedInstance prepareHelpanimationData];
    });

    return sharedInstance;
}

- (void)prepareHelpanimationData
{
    self.helpAnimatioPositions = [NSMutableArray new];
    //1
    NSString *text = NSLocalizedString(@"tap to see animation!", nil);
    NSDictionary *dict = @{@"stwratPoint": [NSValue valueWithCGPoint:CGPointMake(450, 140)],
                           @"labelRect": [NSValue valueWithCGRect:CGRectMake(239, 215, 140, 70)],
                           @"numberOfLines": @3, @"text":text,
                           @"offset":[NSValue valueWithCGPoint:(CGPoint){110,0}],
                           @"imageR": [NSNumber numberWithFloat:1.8],@"labelR": [NSNumber numberWithFloat:-0.12],
                           @"imageName": @"Tutorial-Arrow-L_v2@2x.png"};
    
    [self.helpAnimatioPositions addObject:dict];
    //2
    text = NSLocalizedString(@"tap to build diagram here!", nil);
    dict = @{@"stwratPoint": [NSValue valueWithCGPoint:CGPointMake(770, 305)],
             @"labelRect": [NSValue valueWithCGRect:CGRectMake(690, 480, 100, 70)],
             @"numberOfLines": @3, @"text":text,
             @"offset":[NSValue valueWithCGPoint:(CGPoint){40,-100}],
             @"imageR": [NSNumber numberWithFloat:-2.3],@"labelR": [NSNumber numberWithFloat:0.8],
             @"imageName": @"Tutorial-Arrow-R_v2@2x.png"};
    
    [self.helpAnimatioPositions addObject:dict];
    //3
    text = NSLocalizedString(@"drag the items here to make diagram!", nil);
    dict = @{@"stwratPoint": [NSValue valueWithCGPoint:CGPointMake(700, 250)],
             @"labelRect": [NSValue valueWithCGRect:CGRectMake(720, 430, 200, 70)],
             @"numberOfLines": @3, @"text":text,
             @"offset":[NSValue valueWithCGPoint:(CGPoint){30,100}],
             @"imageR": [NSNumber numberWithFloat:0.05],@"labelR": [NSNumber numberWithFloat:- 0.3],
             @"imageName": @"Tutorial-Arrow-R_v2@2x.png"};
    
    [self.helpAnimatioPositions addObject:dict];
    //4
    text = NSLocalizedString(@"tap to solve the problem!", nil);
    dict = @{@"stwratPoint": [NSValue valueWithCGPoint:CGPointMake(650, 265)],
             @"labelRect": [NSValue valueWithCGRect:CGRectMake(515, 375, 120, 70)],
             @"numberOfLines": @3, @"text":text,
             @"offset":[NSValue valueWithCGPoint:(CGPoint){100,-50}],
             @"imageR": [NSNumber numberWithFloat:-1.8],@"labelR": [NSNumber numberWithFloat:1.2],
             @"imageName": @"Tutorial-Arrow-R_v2@2x.png"};
    
    [self.helpAnimatioPositions addObject:dict];
    //5
    text = NSLocalizedString(@"drag & drop the numbers here to solve the problem!", nil);
    dict = @{@"stwratPoint": [NSValue valueWithCGPoint:CGPointMake(630, 190)],
             @"labelRect": [NSValue valueWithCGRect:CGRectMake(460, 315, 200, 70)],
             @"numberOfLines": @3, @"text":text,
             @"offset":[NSValue valueWithCGPoint:(CGPoint){-70,55}],
             @"imageR": [NSNumber numberWithFloat:-2.0],@"labelR": [NSNumber numberWithFloat:1.0],
             @"imageName": @"Tutorial-Arrow-L_v2@2x.png"};
    
    [self.helpAnimatioPositions addObject:dict];
    //6
    text = NSLocalizedString(@"tap to see the mistake!", nil);
    dict = @{@"stwratPoint": [NSValue valueWithCGPoint:CGPointMake(640, 70)],
             @"labelRect": [NSValue valueWithCGRect:CGRectMake(635, 270, 120, 60)],
             @"numberOfLines": @2, @"text":text,
             @"offset":[NSValue valueWithCGPoint:(CGPoint){-30,-110}],
             @"imageR": [NSNumber numberWithFloat:0],@"labelR": [NSNumber numberWithFloat:-0.4],
             @"imageName": @"Tutorial-Arrow-L_v2@2x.png"};
...
- (void)showHelpAnimatiosIfNeededWithTask:(Task *)task onView:(UIView *)view
{
    [self.animationHelpLabel removeFromSuperview];
    [self.animationImageArow removeFromSuperview];
    
    // 5 - error, handle non error
    if ([task.helpIndex integerValue] < 5 && [self isNeedShowHelpForTask:task]) {
        BOOL needShow = YES;
        NSInteger animationIndex = [task.helpIndex integerValue];
    
        if (animationIndex == 0 && [task.animation length] == 0) {
            task.helpIndex = @(kSchemeTutorial1);
            needShow = NO;
        } else if ([task.helpIndex integerValue] > 2 && (task.status == kTaskStatusSolved ||
                                                         task.status == kTaskStatusSolvedNotAll ||
                                                         task.status == kTaskStatusError)) {
            task.helpIndex = @5;
            needShow = NO;
        }
        
        if([task.helpIndex integerValue] == kSchemeTutorial2 ||  [task.helpIndex integerValue] == kTaskTutorial2) {
            [self hideErrorHelpAnimatios];
        }
        
        [[NSManagedObjectContext contextForCurrentThread] saveToPersistentStoreAndWait];
        
        if (needShow) {
            NSDictionary *dict1 = [self.helpAnimatioPositions objectAtIndex:[task.helpIndex integerValue]];
            CGPoint startPoint1 = [[dict1 objectForKey:@"stwratPoint"] CGPointValue];
            CGRect labelRect =[[dict1 objectForKey:@"labelRect"] CGRectValue];
            
            UIImage *image = [UIImage imageNamed:[dict1 objectForKey:@"imageName"]];
            
            self.animationImageArow = [[UIImageView alloc] initWithImage:image];
            self.animationImageArow.frame = CGRectMake(startPoint1.x, startPoint1.y, image.size.width, image.size.height);
            self.animationHelpLabel = [[UILabel alloc] initWithFrame:labelRect];
            self.animationHelpLabel.textColor = [UIColor whiteColor];
            self.animationHelpLabel.font = [UIFont systemFontOfSize:20.0f];
            self.animationHelpLabel.numberOfLines = [[dict1 objectForKey:@"numberOfLines"] integerValue];
            self.animationHelpLabel.textAlignment = NSTextAlignmentCenter;
            self.animationHelpLabel.text = [dict1 objectForKey:@"text"];
            self.animationHelpLabel.rotation = [[dict1 objectForKey:@"labelR"] floatValue];
            self.animationImageArow.rotation = [[dict1 objectForKey:@"imageR"] floatValue];
            
            [view addSubview:self.animationImageArow];
            [self.animationImageArow startHelpGrowAnimationWithOffset:[[dict1 objectForKey:@"offset"] CGPointValue]];
            [view addSubview:self.animationHelpLabel];
        } else {
            [self showHelpAnimatiosIfNeededWithTask:task onView:view];
        }
    }
}
...
- (void)showErrorTutorialIfNeededOnView:(UIView *)view forTask:(Task *)task
{
    ...
    if ([DataUtils isHaveErrorActionInTask:task]) {
    
        NSDictionary *dict1 = [self.helpAnimatioPositions objectAtIndex:5];
        CGPoint startPoint1 = [[dict1 objectForKey:@"stwratPoint"] CGPointValue];
        CGRect labelRect =[[dict1 objectForKey:@"labelRect"] CGRectValue];
        
        UIImage *image = [UIImage imageNamed:[dict1 objectForKey:@"imageName"]];
        
        self.animationImageArowForError = [[UIImageView alloc] initWithImage:image];
        self.animationImageArowForError.frame = CGRectMake(startPoint1.x, startPoint1.y, image.size.width, image.size.height);
        self.animationHelpLabelForError = [[UILabel alloc] initWithFrame:labelRect];
        self.animationHelpLabelForError.textColor = [UIColor whiteColor];
        self.animationHelpLabelForError.font = [UIFont systemFontOfSize:20.0f];
        self.animationHelpLabelForError.numberOfLines = [[dict1 objectForKey:@"numberOfLines"] integerValue];
        self.animationHelpLabelForError.textAlignment = NSTextAlignmentCenter;
        self.animationHelpLabelForError.text = [dict1 objectForKey:@"text"];
        self.animationHelpLabelForError.rotation = [[dict1 objectForKey:@"labelR"] floatValue];
        self.animationImageArowForError.rotation = [[dict1 objectForKey:@"imageR"] floatValue];
        
        [view addSubview:self.animationImageArowForError];
        [self.animationImageArowForError startHelpGrowAnimationWithOffset:[[dict1 objectForKey:@"offset"] CGPointValue]];
        [view addSubview:self.animationHelpLabelForError];
    }
}
...
{
    [self.animationHelpLabelForError removeFromSuperview];
    [self.animationImageArowForError removeFromSuperview];
}

- (BOOL)isNeedShowHelpForTask:(Task *)task
{
    BOOL isNeed = NO;
    //1-A-1-1 lenght ->7
    if ([[task.identifier substringToIndex:7] isEqualToString:@"1-A-1-1"]) {
        isNeed = YES;
    }
    
    if ([task.helpIndex integerValue] == 5 && task.status != kTaskStatusError) {
        isNeed = NO;
    }
...
- (Scheme *)schemeForTask:(Task *)task
{
    Scheme *scheme = [task.child.schemes match:^BOOL(Scheme *obj) {
        return [obj.identifier isEqualToString:task.identifier];
    }];
    
    return scheme;
}
    
- (BOOL)isSchemeHaveFilledElementFromTask:(Task *)task
{
    return [[[self schemeForTask:task] elements] any:^BOOL(SchemeElement *obj) {
