...
- (void)updateSpendTimes:(NSArray *)spendTimeArray
             withSuccess:(MTHTTPCompletionBlock)successBlock
                 failure:(MTHTTPCompletionBlock)failureBlock
{
    
    static NSString * const kRecordTimeInApp = @"recordtimeinapp";
    NSError *error = nil;
    
    NSMutableDictionary *param = [NSMutableDictionary new];
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:spendTimeArray
                                                       options:NSJSONWritingPrettyPrinted // Pass 0 if you don't care about the readability of the generated string
                                                         error:&error];
    ...
    param = [@{kMTAPINameKey: kRecordTimeInApp,
               kMTAccessTokenKey: [MTHTTPClient MTAccessTokenOrEmptyString],
               @"data": jsonString} mutableCopy];
...
- (void)performRequest:(RequestMethod)method
            withParams:(NSDictionary *)params
   skipIfChildNotAdded:(BOOL)skipIfChildNotAdded
 successExecutionBlock:(MTHTTPSuccessResponseExecutionBlock)successBlock
         andCompletion:(MTHTTPCompletionBlock)completionBlock
{
        [self performRequest:method
                  withParams:params
         skipIfChildNotAdded:skipIfChildNotAdded
               progressBlock:nil
       successExecutionBlock:successBlock
               andCompletion:completionBlock];
}
         ...
         andCompletion:(MTHTTPCompletionBlock)completionBlock
{
    ...
    //child not synced, but request will be performed, not desired behavior for some requests
    if (skipIfChildNotAdded && (![ChildManager sharedInstance].currentChild || [[ChildManager sharedInstance].currentChild.identifier integerValue] == kNewUnsynchronizedChildID)) {
        completionBlock(NO, nil);
        return;
    }
    ...
    //redirect requests to dev server, while app is on Apple submission
    [self.currentOperation setRedirectResponseBlock:^NSURLRequest *(NSURLConnection *connection, NSURLRequest *request, NSURLResponse *redirectResponse) {
...
//        NSLog(@"redirectResponse %@", redirectResponse);
        ...
        if (redirectResponse) {
            ...
            NSMutableURLRequest *requestToRedirect = [connection.originalRequest mutableCopy];
            
            //we get escaped URL, need to unescape it back
            NSString *path = [[request URL] absoluteString];
            path = [path stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            
            [requestToRedirect setURL:[NSURL URLWithString:path]];
            
            NSLog(@"requestToRedirect %@", requestToRedirect);
...
- (BOOL)validateResponseJSON:(id)JSON withError:(NSError * __autoreleasing *)error
{
    ...
    if (!isValid) {
        ...
        if ([JSON valueForKey:kResponseErrorKey]) {
            ...
            //delete current child if failed to set active (it is deleted already from other device)
            if (errorCode == kNoChildFailureCode && self.isRequestsIgnored) {
                
                if ([[ChildManager sharedInstance].currentChild.identifier integerValue] != kNewUnsynchronizedChildID) {
...
+ (BOOL)isExpressionValid:(NSString *)expression expressionType:(ExpressionType)expType
{
    ...
    //\u0600-\u065F\u066A-\u06EF\u06FA-\u06FF - arabic characters http://stackoverflow.com/questions/12518689/regular-expression-not-to-allow-numbers-just-arabic-letters
    
    static NSString * const kUserNameRegularExpression = @"^[A-Za-zА-Яа-я\u0600-\u065F\u066A-\u06EF\u06FA-\u06FF]+([\\wА-Яа-я\u0600-\u065F\u066A-\u06EF\u06FA-\u06FF_\\s.-]){1,30}$";
...
+ (BOOL)validateNickname:(NSString *)nickname withError:(NSError * __autoreleasing *)error
{
    if (![MTHTTPClient isExpressionValid:nickname expressionType:ExpName]) {
        *error = [NSError errorWithString:NSLocalizedString(@"Invalid nickname. You can use only Latin/Cyrillic symbols, whitespaces, digits and symbols '.-_'. Maximum is 15 symbols", nil)];
        return NO;
    }
