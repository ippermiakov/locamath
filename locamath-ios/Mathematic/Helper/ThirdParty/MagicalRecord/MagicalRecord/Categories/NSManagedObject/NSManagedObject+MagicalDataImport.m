...
//    NSLog(@">> before update: %@", self);
    ...
    for (NSString *attributeName in attributes)
    {
        ...
        if (lookupKeyPath) 
        {
            ...
            {
               // NSLog(@"value : %@, attributeName : %@", value, attributeName);
                [self setValue:value forKey:attributeName];
            }
        } 
        else 
        {
            if ([[attributeInfo userInfo][kMagicalRecordImportAttributeUseDefaultValueWhenNotPresent] boolValue]) 
            {
...
{
    //Since we don't use primaryAttribute
...
//    {
    NSManagedObject *managedObject = [self MR_createEntity];
//    }
...
+ (NSArray *) MR_importFromArray:(NSArray *)listOfObjectData inContext:(NSManagedObjectContext *)context
{
    ...
    {    
        [listOfObjectData enumerateObjectsWithOptions:0 usingBlock:^(id obj, NSUInteger idx, BOOL *stop) 
        {
            ...
            if ([context obtainPermanentIDsForObjects:@[dataObject] error:nil])
            {
