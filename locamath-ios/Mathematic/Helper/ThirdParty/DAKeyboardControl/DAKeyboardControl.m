...
+ (void)load
{
    ...
    // Swizzle th 'dealloc' to ensure that all notifications are gone, when view is destroyed
    SEL originalSelector2 = NSSelectorFromString(@"dealloc");
    SEL swizzledSelector2 = @selector(swizzled_dealloc);
    ...
    class_addMethod(self,
					originalSelector2,
					class_getMethodImplementation(self, originalSelector2),
					method_getTypeEncoding(originalMethod2));
	class_addMethod(self,
					swizzledSelector2,
					class_getMethodImplementation(self, swizzledSelector2),
...
- (void)removeKeyboardControl
{
    if (self.keyboardDidMoveBlock) {
        ...
        NSLog(@"!!! removeKeyboardControl %@", self);
