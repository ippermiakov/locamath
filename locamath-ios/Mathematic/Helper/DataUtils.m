...
+ (NSArray *)levelsFromCurrentChild
{
    ...
    //TODO: optimize
//    NSArray *levelsForChild = [[[[ChildManager sharedInstance] currentChild].levels allObjects]
//                               sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
//        return [[obj1 identifier] compare:[obj2 identifier]];
//    }];
    
    return /*levelsForChild*/[Level findAllSortedBy:@"identifier"
                        ascending:YES
                    withPredicate:[builder compoundPredicate]];
}
...
{
    //TODO: optimize
//    NSArray *tasksForChild = [[[[ChildManager sharedInstance] currentChild].tasks allObjects]
//                               sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
//                                   return [[obj1 lastChangeDate] compare:[obj2 lastChangeDate]];
//                               }];
//    
//    
...
+ (NSNumber *)countOfSolvedActionsFromActions:(NSArray *)actions
{
    ...
    for (Action *action in actions) {
        if (action.error == kActionErrorTypeNone && action.answer.length > 0) {
            count++;
        }
    }
...
+ (NSArray *)childs
{
    return [Child findAllSortedBy:@"selectionDate" ascending:NO];
}
...
+ (BOOL)isHaveErrorActionInTask:(Task *)task
{
    return [task.actions any:^BOOL(Action *obj) {
        return obj.error != kActionErrorTypeNone;
    }];
}

#pragma mark - SpendTimeStatistic

+ (SpendTimeStatistic *)spendTimeStatisticForCurrentChildWithDateString:(NSString *)date
{
    SpendTimeStatistic * spendTimeStatistic = [[[ChildManager sharedInstance].currentChild.spendTimes allObjects] match:^BOOL(SpendTimeStatistic *obj) {
        return [obj.statisticDate isEqualToString:date];
    }];
    
    return spendTimeStatistic;
}

+ (NSString *)presentDateString
{
...
+ (BOOL)isArabicLocale
{
    NSString *arabicID = @"ar";
    
    return [[[NSLocale preferredLanguages] objectAtIndex:0] isEqualToString:arabicID];
}

+ (NSString *)corretcLocaleTableTextIfNeededWithString:(NSString *)text
{
//    NSMutableString *reflectWords = [NSMutableString new];
//    
...
//            [reflectWords appendString:words[i]];
//            if (i > 0) {
//                //check if need add dot!
//                if ([words[i - 1] integerValue] != 0 && i == 1) {
//                    [reflectWords appendString:@"."];
//                } else {
//                    [reflectWords appendString:@" "] ;
//                }
//            }
...
//        }
//    } else {
//        reflectWords = [text mutableCopy];
//    }
    
    return text/*reflectWords*/;
}
