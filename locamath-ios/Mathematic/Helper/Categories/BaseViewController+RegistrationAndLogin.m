...
- (void)showChildSelectionScreen
{
    ...
    } else if ([[ChildManager sharedInstance].currentChild.identifier integerValue] == kNewUnsynchronizedChildID && [DataUtils currentParent] != nil) {
        
        [[ChildManager sharedInstance] loadChildsWithSuccess:^{
            NSLog(@"updated childs list for unsync childs");
        } failure:^(NSError *error) {
            NSLog(@"failed to update childs list for unsync childs");
        }];
    } else {
...
{
    [self selectChildWithTraining:isWithTraining canAddChilds:YES];
}
...
- (void)selectChildWithTraining:(BOOL)isWithTraining canAddChilds:(BOOL)canAddChilds
{
    ...
    if ([ChildManager sharedInstance].currentChild == nil && [DataUtils currentParent] != nil) {
        ...
        chooseChildPopupViewController.canAddChilds = canAddChilds;
        
        if (![GameManager sharedInstance].game.skipStatisticScreen) {
            chooseChildPopupViewController.onFinish = ^{
                [self presentViewController:[[StatisticViewController alloc]
...
- (void)handleIfParentIsRegisteredWithError:(NSError *)error andUserInfo:(NSDictionary *)userInfo
{
    ...
    if (error.code == kParentAlreadyExistsErrorCode) {
        ...
        [UIAlertView showAlertViewWithTitle:NSLocalizedString(@"Attention", nil)
                                    message:message cancelButtonTitle:NSLocalizedString(@"Cancel", nil)
                          otherButtonTitles:[NSArray arrayWithObjects:NSLocalizedString(@"OK", nil), nil]  handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
        ...
        if ([[MTHTTPClient sharedMTHTTPClient] isParentAuthentificated] || (![[MTHTTPClient sharedMTHTTPClient] isReachable] && [DataUtils currentParent])) {
            ...
            registerViewController.onFinish = ^{
                ...
                if ([[ChildManager sharedInstance].currentChild.name isEqualToString:kDefaultChildName]) {
                    ...
                    chooseAvatarViewController.onFinish = complition;
                    ...
                    NSLog(@"current child name  %@", [ChildManager sharedInstance].currentChild.name);
