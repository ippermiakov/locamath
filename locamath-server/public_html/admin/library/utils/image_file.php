...
class image_file {
  ...
  function image_file($imgfile) {
    ...
    if (image_lib_supported()) {
      $this->valid = true;
      if ($this->image = ImageCreateFromGIF ($imgfile))
        $this->format = "GIF";
      elseif ($this->image = ImageCreateFromJPEG ($imgfile)) {
        $this->format = "JPEG";
        $this->dpi = $this->get_jpeg_dpi($imgfile);
      }
      elseif ($this->image = ImageCreateFromPNG ($imgfile))
        $this->format = "PNG";
...
function copy_image_part($source_file, $source_x, $source_y, $width, $height, $destination_file = null, $overwrite = false) {
  ...
  if (file_exists($source_file)) {
    if (!file_exists($destination_file) or $overwrite) {
      $image_file = new image_file($source_file);
      if ($image_file->valid) { 
        if (($image_file->height() >= $height) || ($imimage_file->height() >= $height) || ($image_file->width() >= $width)) {
          if ($image_file->copy($source_x, $source_y, min($width, $image_file->width()),($height, $image_file->height()), $destination_file))
            return $destination_file;
          else
            return null;  
        } else {
