...
class Page_Frame_Decorator extends Frame_Decorator {
   ...
   * @param float $h
   */
  function set_containing_block($x = null, $y = nut $x
   * @param float $y
   * @param float $w
   * @param float $h
   */
...
//       echo "block: break allowed\n";      
    ...
    // Inline frames (2):    
    else if ( in_array($display, CSSStyle::$INLINE_TYPES) ) {
...
//         echo "following last child.\n";
        ...
        if ( in_array($next->get_style()->display, CSSStyle::$TABLE_TYPES) )
          $this->_in_table++;
...
//         echo "following prev sibling.\n";
        ...
        if ( in_array($next->get_style()->display, CSSStyle::$TABLE_TYPES) &&
             !in_array($iter->get_style()->display, CSSStyle::$TABLE_TYPES) )
          $this->_in_table++;

        else if ( !in_array($next->get_style()->display, CSSStyle::$TABLE_TYPES) &&
                  in_array($iter->get_style()->display, CSSStyle::$TABLE_TYPES) )
          $this->_in_table--;
...
//         echo "following parent.\n";
        ...
        if ( in_array($iter->get_style()->display, CSSStyle::$TABLE_TYPES) )
          $this->_in_table--;
         $iter = $next;
        $flg = true;
        continue;
