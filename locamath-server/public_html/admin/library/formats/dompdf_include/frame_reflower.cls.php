...
abstract class Frame_Reflower {
  ...
  protected function _collapse_margins() {
    ...
    while ( $n && !in_array($n->get_style()->display, CSSStyle::$BLOCK_TYPES) )
    ...
    while ( $f && !in_array($f->get_style()->display, CSSStyle::$BLOCK_TYPES) )
  ...
  function get_min_max_width() {
    ...
    for ( $iter = $this->_frame->get_children()->getIterator();
      ...
      while ( $iter->valid() && in_array( $iter->current()->get_style()->display, CSSStyle::$INLINE_TYPES ) ) {
