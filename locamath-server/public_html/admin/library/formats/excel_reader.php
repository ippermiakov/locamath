...
require_once(dirname(__FILE__).'/ole_reader.inc');

//define('excel_reader_HAVE_ICONV', function_exists('iconv'));
//define('excel_reader_HAVE_MB', function_exists('mb_convert_encoding'));

define('excel_reader_BIFF8', 0x600);
define('excel_reader_BIFF7', 0x500);
define('excel_reader_WorkbookGlobals', 0x5);
define('excel_reader_Worksheet', 0x10);

define('excel_reader_Type_BOF', 0x809);
define('excel_reader_Type_EOF', 0x0a);
define('excel_reader_Type_BOUNDSHEET', 0x85);
define('excel_reader_Type_DIMENSION', 0x200);
define('excel_reader_Type_ROW', 0x208);
define('excel_reader_Type_DBCELL', 0xd7);
define('excel_reader_Type_FILEPASS', 0x2f);
define('excel_reader_Type_NOTE', 0x1c);
define('excel_reader_Type_TXO', 0x1b6);
define('excel_reader_Type_RK', 0x7e);
define('excel_reader_Type_RK2', 0x27e);
define('excel_reader_Type_MULRK', 0xbd);
define('excel_reader_Type_MULBLANK', 0xbe);
define('excel_reader_Type_INDEX', 0x20b);
define('excel_reader_Type_SST', 0xfc);
define('excel_reader_Type_EXTSST', 0xff);
define('excel_reader_Type_CONTINUE', 0x3c);
define('excel_reader_Type_LABEL', 0x204);
define('excel_reader_Type_LABELSST', 0xfd);
define('excel_reader_Type_NUMBER', 0x203);
define('excel_reader_Type_NAME', 0x18);
define('excel_reader_Type_ARRAY', 0x221);
define('excel_reader_Type_STRING', 0x207);
define('excel_reader_Type_FORMULA', 0x406);
define('excel_reader_Type_FORMULA2', 0x6);
define('excel_reader_Type_FORMAT', 0x41e);
define('excel_reader_Type_XF', 0xe0);
define('excel_reader_Type_BOOLERR', 0x205);
define('excel_reader_Type_UNKNOWN', 0xffff);
define('excel_reader_Type_NINETEENFOUR', 0x22);
define('excel_reader_Type_MERGEDCELLS', 0xE5);

define('excel_reader_utcOffsetDays' , 25569);
define('excel_reader_utcOffsetDays1904', 24107);
define('excel_reader_msInADay', 24 * 60 * 60);

//define('excel_reader_DEF_NUM_FORMAT', "%.2f");
define('excel_reader_DEF_NUM_FORMAT', "%s");
...
//class excel_reader extends PEAR {
class excel_reader {
    ...
    var $_defaultFormat = excel_reader_DEF_NUM_FORMAT;
    ...
    */
    ...
    function read($sFileName) {
        ...
        if($res === false) {
        	...
        	// bad file
        		return false; //die('The filename ' . $sFileName . ' is not readable');	
        	}
...
//		var_dump($res);		
    ...
    function _parse(){
        ...
        if (($version != excel_reader_BIFF8) && ($version != excel_reader_BIFF7)) {
            return false;
        }

        if ($substreamType != excel_reader_WorkbookGlobals){
            return false;
        }
        ...
        while ($code != excel_reader_Type_EOF){
            switch ($code) {
                case excel_reader_Type_SST:
                    //echo "Type_SST\n";
                     $spos = $pos + 4;
...
//                                              echo "Str $i = $retstr\n";
                ...
                case excel_reader_Type_FILEPASS:
                    return false;
                    break;
                case excel_reader_Type_NAME:
                    //echo "Type_NAME\n";
                    break;
                case excel_reader_Type_FORMAT:
                        $indexCode = ord($this->data[$pos+4]) | ord($this->data[$pos+5]) << 8;

                        if ($version == excel_reader_BIFF8) {
                ...
                case excel_reader_Type_XF:
                        ...
                        //echo "\n";
                    break;
                case excel_reader_Type_NINETEENFOUR:
                    //echo "Type.NINETEENFOUR\n";
                    $this->nineteenFour = (ord($this->data[$pos+4]) == 1);
                    break;
                case excel_reader_Type_BOUNDSHEET:
                    //echo "Type.BOUNDSHEET\n";
                        ...
                        }elseif ($version == excel_reader_BIFF7){
    ...
    function _parsesheet($spos){
        ...
        if (($version != excel_reader_BIFF8) && ($versiype = ord($this->data[$spos + 6]) | ord($this->data[$spos + 7])<<8;

        if (($version != excel_reader_BIFF8) && ($version != excel_reader_BIFF7)) {
            return -1;
        }

        if ($substreamType != excel_reader_Worksheet){
            return -2;
        }
        //echo "Start parse code=".base_convert($code,10,16)." version=".base_convert($version,10,16)." substreamType=".base_convert($substreamType,10,16).""."\n";
        $spos += $length + 4;
...
//            $r = &$this->file->nextRecord();
            ...
            if ($lowcode == excel_reader_Type_EOF) break;
            ...
            switch ($code) {
                case excel_reader_Type_DIMENSION:
                    //echo 'Type_DIMENSION ';
                    if (!isset($this->numRows)) {
                        if (($length == 10) ||  ($version == excel_reader_BIFF7)){
                    ...
                    break;
                case excel_reader_Type_MERGEDCELLS:
                    ...
                    break;
                case excel_reader_Type_RK:
                case excel_reader_Type_RK2:
                    //echo 'excel_reader_Type_RK'."\n";
                    ...
                    break;
                case excel_reader_Type_LABELSST:
                        $row        = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                        $column     = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
			...
			//var_dump($this->sst);
                ...
                case excel_reader_Type_NUMBER:
                    ...
                    break;
                case excel_reader_Type_FORMULA:
                case excel_reader_Type_FORMULA2:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
					...
					break;                    
                case excel_reader_Type_BOOLERR:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
                    ...
                    //echo 'Type_BOOLERR '."\n";
                    break;
                case excel_reader_Type_ROW:
                case excel_reader_Type_DBCELL:
                case excel_reader_Type_MULBLANK:
                    break;
                case excel_reader_Type_LABEL:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
    ...
    function createDate($numValue){
        if ($numValue > 1){
            $utcDays = $numValue - ($this->nineteenFour ? excel_reader_utcOffsetDays1904 : excel_reader_utcOffsetDays);
            $utcValue = round($utcDays * excel_reader_msInADay);
            ...
            $secs = floor($numValue * excel_reader_msInADay) - $hours * 60 * 60 - $mins * 60;
        ...
        if ($this->_defaultEncoding){
        	switch ($this->_encoderFunction){
        		case 'iconv' : 	$result = iconv('UTF-16LE', $this->_defaultEncoding, $string);
        						b      		case 'mb_convert_encoding' : 	$result = mb_convert_encoding($string, $this->_defaultEncoding, 'UTF-16LE' );
        						break;
        	}
        }
        return $result;
