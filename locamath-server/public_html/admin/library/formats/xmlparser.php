    ...
    */
    ...
    function Parse()
    {
        ...
        if (!xml_parse($this->parser, $this->xml)) {
          ...
          $this->errorColumn = xml_get_current_column_number($this->parser);
          $this->errorInfo = xml_get_error_code($this->parser);
        } 
    ...
    function CharacterData($parser, $data)
    {
        ...
        //Assign data to it
        if ($tag) @eval('$this->'.$tag.'->tagData .= trim($data);');
    }
    
    /**
     * XPATH function - Returns the XMLTag object if found, false otherwise.
     *
     * @param string $xpath
     * @param resource $tag_element
     */
    function xpath($xpath, $tag_element = false) {
        
        $names = split ("/", $xpath);
        if (count($this->stack) == 0)  $this->stack = array('document');
        //echo '<p>#'.$names[count($this->stack)-1].'</p>';
        $tag = $this->GetStackLocation();
        $main_tags = array();
        if (!$tag_element) {
            //debug('$main_tags = $this->'.$tag.'->tagChildren;'); 
            @eval('$main_tags = $this->'.$tag.'->tagChildren;'); 
        } else {
            //debug('$main_tags = $tag_element->tagChildren;'); 
            @eval('$main_tags = $tag_element->tagChildren;'); 
        }
        //print_r(var_dump($main_tags));
        if (count($main_tags)>0)
            foreach ($main_tags as $element) {
                if (strcasecmp($element->tagName, $names[count($this->stack)-1]) == 0) {
                    if (count($names)>count($this->stack)) {
                        $this->stack[] = strtolower($element->tagName);
                        return $this->xpath($xpath, $element);
                    } else {
                        $this->stack = array();
                        return $element;
                    }
