...
class  Cpdf {
      ...
      if  (count($o['info']['pages'])) {
        ...
        if  ( (isset($o['info']['fonts']) &&  count($o['info']['fonts'])) ||
          ...
          if  (isset($o['info']['mediaBox'])) {
            ...
            $res.= "\n/MediaBox ["._fixed_sprintf('%.3f', $tmp[0]) .' '._fixed_sprintf('%.3f', $tmp[1]) .' '._fixed_sprintf('%.3f', $tmp[2]) .' '._fixed_sprintf('%.3f', $tmp[3]) .']';
          }
        }
  ...
  function  setCurrentFont() {
    ...
    //     $this->selectFont(dirname(dirname(dirname(__FILE__))).'/_fonts/dompdf/Helvetica.afm');
    //   }
  ...
  function  setColor($r, $g, $b, $force =  0) {
    ...
    if  ($r >=  0 &&  ($force ||  $r !=  $this->currentColour['r'] ||  $g !=  $this->currentColour['g'] ||  $b !=  $this->currentColour['b'])) {
      ...
      $this->objects[$this->currentContents]['c'].=  "\n"._fixed_sprintf('%.3f', $r) .' '._fixed_sprintf('%.3f', $g) .' '._fixed_sprintf('%.3f', $b) .' rg';
  ...
  function  line($x1, $y1, $x2, $y2) {
    ...
    $this->objects[$this->currentContents]['c'] .=
      "\n"._fixed_sprintf('%.3f', $x1) .' '._fixed_sprintf('%.3f', $y1) .' m '._fixed_sprintf('%.3f', $x2) .' '._fixed_sprintf('%.3f', $y2) .' l S';
  }
  ...
  function  curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3) {
    ...
    $this->objects[$this->currentContents]['c'] .=
      "\n"._fixed_sprintf('%.3f', $x0) .' '._fixed_sprintf('%.3f', $y0) .' m '._fixed_sprintf('%.3f', $x1) .' '._fixed_sprintf('%.3f', $y1);

    $this->objects[$this->currentContents]['c'] .=
      ' '._fixed_sprintf('%.3f', $x2) .' '._fixed_sprintf('%.3f', $y2) .' '._fixed_sprintf('%.3f', $x3) .' '._fixed_sprintf('%.3f', $y3) .' c S';
  }
  ...
  function  ellipse($x0, $y0, $r1, $r2 =  0, $angle =  0, $nSeg =  8, $astart =  0, $afinish =  360, $close =  1, $fill =  0) {
    ...
    if  ($angle !=  0) {
      ...
      $tmp  =  "\n q ";
      $tmp .=  _fixed_sprintf('%.3f', cos($a)) .' '._fixed_sprintf('%.3f', (-1.0*sin($a))) .' '._fixed_sprintf('%.3f', sin($a)) .' '._fixed_sprintf('%.3f', cos($a)) .' ';
      $tmp .=  _fixed_sprintf('%.3f', $x0) .' '._fixed_sprintf('%.3f', $y0) .' cm';
    ...
    $this->objects[$this->currentContents]['c'] .=  "\n"._fixed_sprintf('%.3f', $a0) .' '._fixed_sprintf('%.3f', $b0) .' m ';
    ...
    for  ($i = 1; $i <=  $nSeg; $i++) {
      ...
      $this->objects[$this->currentContents]['c']
        .=  "\n"._fixed_sprintf('%.3f', ($a0+$c0*$dtm)) .' '._fixed_sprintf('%.3f', ($b0 + $d0 * $dtm));

      $this->objects[$this->currentContents]['c'] .=
        ' '._fixed_sprintf('%.3f', ($a1-$c1*$dtm)) .' '._fixed_sprintf('%.3f', ($b1-$d1*$dtm)) .' '._fixed_sprintf('%.3f', $a1) .' '._fixed_sprintf('%.3f', $b1) .' c';
  ...
  function  polygon($p, $np, $f =  0) {
    ...
    $this->objects[$this->currentContents]['c'].=  _fixed_sprintf('%.3f', $p[0]) .' '._fixed_sprintf('%.3f', $p[1]) .' m ';

    for  ($i =  2; $i < $np * 2; $i =  $i + 2) {

      $this->objects[$this->currentContents]['c'].=  _fixed_sprintf('%.3f', $p[$i]) .' '._fixed_sprintf('%.3f', $p[$i+1]) .' l ';
    }
  ...
  function  filledRectangle($x1, $y1, $width, $height) {
    ...
    $this->objects[$this->currentContents]['c'].=  "\n"._fixed_sprintf('%.3f', $x1) .' '._fixed_sprintf('%.3f', $y1) .' '._fixed_sprintf('%.3f', $width) .' '._fixed_sprintf('%.3f', $height) .' re f';
  }
  ...
  function  rectangle($x1, $y1, $width, $height) {
    ...
    $this->objects[$this->currentContents]['c'].=  "\n"._fixed_sprintf('%.3f', $x1) .' '._fixed_sprintf('%.3f', $y1) .' '._fixed_sprintf('%.3f', $width) .' '._fixed_sprintf('%.3f', $height) .' re S';
  }
  ...
  function  getFontHeight($size) {
    ...
    if  (!$this->numFonts) {
      ...
      $this->selectFont(dirname(dirname(dirname(__FILE__))).'/_fonts/dompdf/Helvetica');
    }
  ...
  function  addText($x, $y, $size, $text, $angle =  0, $wordSpaceAdjust =  0) {
    ...
    if  ($angle ==  0) {
      ...
      $this->objects[$this->currentContents]['c'].=  "\n".'BT '._fixed_sprintf('%.3f', $x) .' '._fixed_sprintf('%.3f', $y) .' Td';
      ...
      $tmp.=  _fixed_sprintf('%.3f', $x) .' '._fixed_sprintf('%.3f', $y) .' Tm';
    ...
    if  ($wordSpaceAdjust !=  0 ||  $wordSpaceAdjust !=  $this->wordSpaceAdjust) {
      ...
      $this->objects[$this->currentContents]['c'].=  ' '._fixed_sprintf('%.3f', $wordSpaceAdjust) .' Tw';
    }
    ...
    /*
     ...
     $this->objects[$this->currentContents]['c'] .= ' /F'.$this->currentFontNum.' '._fixed_sprintf('%.1f',$size).' Tf ';
     ...
     if ($angle == 0){
     $this->objects[$this->currentContents]['c'] .= "\n".'BT '._fixed_sprintf('%.3f',$xp).' '._fixed_sprintf('%.3f',$yp).' Td';
     } else {
     $a = deg2rad((float)$angle);
     $tmp = "\n".'BT ';
     $tmp .= _fixed_sprintf('%.3f',cos($a)).' '._fixed_sprintf('%.3f',(-1.0*sin($a))).' '._fixed_sprintf('%.3f',sin($a)).' '._fixed_sprintf('%.3f',cos($a)).' ';
     $tmp .= _fixed_sprintf('%.3f',$xp).' '._fixed_sprintf('%.3f',$yp).' Tm';
     $this->objects[$this->currentContents]['c'] .= $tmp;
     }
     if ($wordSpaceAdjust != 0 || $wordSpaceAdjust != $this->wordSpaceAdjust){
     $this->wordSpaceAdjust = $wordSpaceAdjust;
  ...
  function  addPngFromFile($file, $x, $y, $w =  0, $h =  0) {
    ...
    $this->objects[$this->currentContents]['c'].=  "\n"._fixed_sprintf('%.3f', $w) ." 0 0 "._fixed_sprintf('%.3f', $h) ." "._fixed_sprintf('%.3f', $x) ." "._fixed_sprintf('%.3f', $y) ." cm";
...
function _fixed_sprintf($format, $value) {

  return str_replace(',', '.', sprintf($format, $value));
